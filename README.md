...
- The notebook and rebuild script first attempt the programmatic download/API steps, then use the committed raw files as cache fallbacks if a live source is unavailable during review. The S&P 500 raw file is also protected by a SHA-256 check, so upstream changes in the moving GitHub source do not silently alter the submitted analysis.
...
- The validation script enforces full year coverage from 2000-2023, with documented partial-year exceptions: the 2023 S&P source rows contain placeholder values after September, so S&P annual metrics are aligned to the 9 valid months with available price/CPI/rate/PE10 fields, while dividend and earnings averages use the 6 months with available values. The merged dataset labels this explicitly with `sp500_months_used`, `dividend_months_used`, `earnings_months_used`, and `sp500_coverage_label`. Cached S&P rows after 2023 are excluded from the analysis window but also contain unavailable-data placeholders in the same non-price fields. Gold futures data begins partway through 2000, so gold annual-average YoY changes are suppressed for 2000 and 2001.
- `dividend_yield` is computed from months where both `Dividend` and `SP500` are available, so 2023 does not divide a 6-month dividend average by a 9-month S&P average.
- `sp500_annual_avg_yoy_change` and `gold_annual_avg_yoy_change` are year-over-year percentage changes in **annual average price levels**, not true calendar-year total returns. S&P annual-average YoY is suppressed for the partial 2023 S&P observation; gold annual-average YoY is suppressed for 2000 and 2001 because gold starts partway through 2000. Use month-end/year-end values or total-return series if investment return precision is required.
- Correlations use pairwise complete observations. Most variables have 24 annual observations, while `sp500_annual_avg_yoy_change` and `gold_annual_avg_yoy_change` each have 22 non-null observations after documented boundary-year and partial-year suppression.
- Inflation-regime summaries report both total years in each regime and the non-null observation counts used for each YoY average, because S&P and gold YoY series have boundary-year and partial-year missing values.
...
- The merged annual dataset contains 24 observations (2000-2023). It stores both `gdp_growth_lag1` and `gdp_growth_lead1`; the lead-lag visualization uses `gdp_growth_lead1`. PE10 correlation and regression conclusions are not materially changed when the partial 2023 S&P observation is excluded. Treat correlations, regression, and lead-lag conclusions as exploratory rather than causal.
...
3. **Gold Proxy vs. Equities**: Inflation-regime analysis compares gold futures/proxy price changes with S&P 500 annual-average price changes. Gold price changes are highest in the moderate-inflation group rather than the high-inflation group, but the group-level averages use the non-null YoY counts documented in the notebook, including suppression of the partial 2023 S&P YoY value, so this is exploratory and should not be read as a full inflation-hedge test.
