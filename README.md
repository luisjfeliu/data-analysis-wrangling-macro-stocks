...
This project explores how macroeconomic growth and inflation trends in the United States relate to historical S&P 500 stock market valuation, dividend yields, long-term interest rates, and a gold futures price proxy. By programmatically gathering, cleaning, and merging three distinct datasets, the notebook performs exploratory analysis of relationships between macroeconomic health and financial asset valuation/price measures.
...
- **US Macroeconomic Data (JSON)**: Fetched from the World Bank Indicators API for annual US GDP growth and CPI inflation from 2000 to 2023.
- **Gold Futures Price Proxy (API)**: Programmatically retrieved from Yahoo Finance via the `yfinance` library using `GC=F`. Note that `GC=F` is a futures-based Yahoo Finance symbol, so the project should describe it as a gold futures/proxy series, not as physical spot gold.

## Important Data Notes

- `data/world_bank_clean.csv` is expected to contain one row per US year. A previous version included repeated World Bank aggregate income-group rows with blank ISO codes; the current file is restricted to `countryiso3code == "USA"`.
- The notebook's `sp500_annual_return` and `gold_annual_return` columns are year-over-year percentage changes in **annual average price levels**, not true calendar-year total returns. Use month-end/year-end values or total-return series if investment return precision is required.
- `earnings_yield` is computed as `100 / PE10`, so it is a **CAPE yield proxy**, not a conventional current earnings yield.
- `equity_risk_premium` is a rough **CAPE yield minus 10-year yield spread**, not a full expected equity risk premium model.
...
The notebook produces several exploratory plots:
